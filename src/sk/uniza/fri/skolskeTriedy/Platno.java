package sk.uniza.fri.skolskeTriedy;
...
public class Platno {
    ...
    public static Platno dajPlatno() {
        if (Platno.platnoSingleton == null) {
            Platno.platnoSingleton = new Platno("BlackJack", 800, 600,
                                         Color.green);
        }
    ...
    private Platno(String titulok, int sirka, int vyska, Color pozadie) {
        ...
        /**
         * Okno ktoré reaguje na rozhodnutie hráča
         */
        this.frame.addWindowListener(new WindowAdapter() {
            @Override
            public void windowClosing(WindowEvent e) {
                int moznost = JOptionPane.showConfirmDialog(null, "Naozaj chces odísť? Tvoje neulozene údaje sa stratia.");
                if (moznost == 0) {
                    Platno.dajPlatno().frame.setDefaultCloseOperation(WindowConstants.EXIT_ON_CLOSE);
                } else if (moznost == 1) {
                    Platno.dajPlatno().frame.setDefaultCloseOperation(WindowConstants.DO_NOTHING_ON_CLOSE);
                } else if (moznost == 2) {
                    Platno.dajPlatno().frame.setDefaultCloseOperation(WindowConstants.DO_NOTHING_ON_CLOSE);
                }
            }
    ...
    /**
     * Ak hráč nemá žiadne peniaze tak aplikácia zatvorí okno
     */
    public void zatvorPlatno() {
        JOptionPane.showMessageDialog(null, "Nemáš žiadne peniaze.");
        System.exit(0);
    }
